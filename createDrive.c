...
void create_line_follow_left(int dist, int speed, int side) {
    ...
    while(abs(get_create_distance()) < abs(dist)) {
        ...
        int avg = (lWhite + lBlack)/2;

        if (side == LEFT) {
            create_drive_direct(200+(.0015*speed*(lCliff() - avg)), 200+(.0015*speed*(avg - lCliff())));
        } else if (side == RIGHT) {
            create_drive_direct(200+(.0015*speed*(avg - lCliff())), 200+(.0015*speed*(lCliff() - avg)));
        }
    }
    create_stop();
}
...
void create_square_up_fcliff_white(int speed)
{
    double offset = 0;
    while (rfCliff() < (rfBlack+rfWhite)/2 || lfCliff() < (lfBlack+lfWhite)/2)
    {
